import {FlatList, Text, View, TouchableOpacity, SafeAreaView, TouchableOpacityComponent} from "react-native";
...
const CommunityDisplayByFilter = ({heading, optionList, userList, filterMaxCards, navigation}) => {
    ...
    useEffect(() => {
        ...
        userList.map((item) => {
            let check = 0
            if (item[heading] === selectedOption || item[heading].includes(selectedOption)) check++
    ...
    return (
        <View key={heading}>
            ...
            <View style={styles.optionWrapper}>
                <FlatList data={options} extraData={options} keyExtractor={option => option.id} horizontal={true}
                          renderItem={(option) => {
                              if (option.item.name !== selectedOption) return (
